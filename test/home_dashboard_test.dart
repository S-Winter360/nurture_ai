...
void main() {
  ...
  group('Phase E3 Home Dashboard Integration Tests', () {
    late Database db;
    ...
    setUp(() async {
      dbPath = join(await getDatabasesPath(), 'home_test.db');
    ...
    test('Greeting retrieves stored caregiver name Ama Mensah', () async {
      final user = await userRepo.getUser('user_ama_01');
      expect(user, isNotNull);
      expect(user!.name, 'Ama Mensah');
    });

    test('Pregnancy hero card calculates exact 26 weeks from stored LMP', () async {
      final preg = await pregRepo.getActivePregnancy('mother_ama_01');
      expect(preg, isNotNull);
      final gestAge = GestationalAge.fromLMP(preg!.lastMenstrualPeriod);
      expect(gestAge.displayWeek, 26);
    });

    test('Priority ANC appointment is retrieved from SQLite', () async {
      final apts = await aptRepo.getAppointmentsForProfile('mother_ama_01');
      expect(apts, isNotEmpty);
      expect(apts.first.appointmentType, 'ANC Visit');
      expect(apts.first.facilityName, 'City Hospital, Tamale');
    });

    test('Care task timeline is retrieved from SQLite', () async {
      final tasks = await taskRepo.getTasksForProfile('mother_ama_01');
      expect(tasks, isNotEmpty);
      expect(tasks.first.title, 'ANC visit');
    });
  });
}