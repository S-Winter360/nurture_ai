...
import 'package:nurture_ai/repositories/vaccination_repository.dart';
import 'package:nurture_ai/repositories/growth_repository.dart';
...
void main() {
  ...
  group('Phase E4 Child Data Integration Tests', () {
    late Database db;
    late VaccinationRepository vaxRepo;
    late GrowthRepository growthRepo;
    late String dbPath;

    setUp(() async {
      dbPath = join(await getDatabasesPath(), 'child_test.db');
    ...
    test('Fetch vaccinations for child Mariam', () async {
      final vaccines = await vaxRepo.getVaccinationsForChild('child_mariam_01');
      expect(vaccines, isNotEmpty);
      expect(vaccines.any((v) => v.vaccineName == 'BCG' && v.status == 'completed'), isTrue);
    });

    test('Fetch growth records for child Leo', () async {
      final growth = await growthRepo.getGrowthRecordsForChild('child_leo_01');
      expect(growth, isNotEmpty);
      expect(growth.first.weightKg, 14.5);
    });
  });
}